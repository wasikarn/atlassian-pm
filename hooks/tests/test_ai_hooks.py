...
class TestIntentDetect(unittest.TestCase):
    ...
    def test_detects_story_creation_english(self):
        with patch("plugin.ai.intent_detect.claude_call_json", return_value={"intent": "story"}):
    ...
    def test_returns_none_for_unrelated_prompt(self):
        with patch("plugin.ai.intent_detect.claude_call_json", return_value={"intent": "none"}):
...
class TestAcCoverage(unittest.TestCase):
    ...
    def test_returns_score_when_claude_responds(self):
        with patch("plugin.ai.ac_coverage.claude_call_json", return_value={"score": 72}):
