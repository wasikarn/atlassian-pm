...
**1. Dependency Graph (Mermaid)** — `graph LR` with red fill (`fill:#ff6b6b`) for critical path items. Render via `/atlassian-pm:apm-pretty-mermaid` → ASCII for Jira comment; paste raw `.mmd` into Forge Mermaid macro for Confluence page.
