...
| --------- | --------- | --- |
| Definition of Ready (DoR) | Phases 2-4 combined checklist | DoR is a team agreement that a task is clear, actionable, and aligned before development starts; the 90% threshold operationalises "ready" as a number rather than a feeling |
| INVEST validation (William C. Wake) | Phase 3 Task-type checks: INVEST criteria (6 points) | Each INVEST criterion maps to a specific failure mode: non-Independent → blocked sprint; non-Testable → QA cannot close; non-Small → multi-sprint bleed |
...
| Hierarchy alignment (A1-A6) | Phase 4 `--with-subtasks` | Task alignment is the agile equivalent of requirements traceability; unmapped ACs (A1 failure) are the leading cause of features shipped without all acceptance criteria being met |
...
- **DoR pass threshold:** 90% overall (Technical + Quality combined); issues below this should not enter sprint planning — the cost of fixing a task after sprint start is 5-8x the cost of fixing it during refinement
- **Alignment score target:** A1-A6 all passing (`--with-subtasks` mode); A1 (AC ↔ Task Coverage) and A2 (Service Tag Match) are the highest-value checks — a single failure in either predicts a 60%+ chance of incomplete task delivery
...
- Always run `--with-subtasks` when verifying a Task that has subtasks; running without it skips A1-A6 entirely and misses the most impactful alignment gaps
...
- If A4 (Epic ↔ Task Fit) fails → do not auto-fix; scope misalignment between task and epic requires a human decision about whether to descope the task or expand the epic's must-have list
- If A1 (AC ↔ Task Coverage) shows an unmapped AC → create the missing task via `/create-task` rather than patching the task text; the gap is in implementation planning, not in task writing
...
- Standalone tasks (no epic parent) legitimately skip A4 — this is expected behavior, not a defect; flag as info only

### Common Failure Modes

| Symptom | Root Cause | Expert Fix |
| ------- | --------- | --------- |
| A1 fails (AC has no task) | Task was created with `/create-task` but task design was never completed | Run `/create-task {{PROJECT_KEY}}-XXX` to design and create the missing tasks; do not patch the task to remove the AC |
| A2 fails (service tag mismatch) | Task's "Services Impacted" section lists a service but no task carries that service tag | Either add the missing task for the unlisted service, or remove the service from the task's impact list if it was incorrectly included |
...
| S1-S6 quality score low after `--fix` | ADF surgery corrected structure but content quality (language, AC format) was not addressed | After structural fix, run `/update-task {{PROJECT_KEY}}-XXX` to rewrite ACs in Given-When-Then format |
| A6 flagged as "unclear mapping" | Confluence Tech Note uses different terminology than the Jira task ACs | Update the Tech Note section headings to match the AC identifiers (AC1, AC2…); do not change the Jira task to match Confluence — Jira is authoritative for AC text |
