...
Setup is engineered as an **idempotent provisioning script** — a concept from infrastructure-as-code where running an operation N times produces the same result as running it once. This matters because developer environments are re-provisioned constantly (plugin reinstalls, machine migrations, onboarding). The Phase 1 detection scan + fast-path skip pattern eliminates the most common onboarding fear: "will re-running this break what I already have?"

### Industry Frameworks Used

| Framework | Applied In | Why |
| --- | --- | --- |
| 12-Factor App — Factor III (Config) | Phase 5 step 1: credentials in `~/.config/atlassian/.env`, not code | Config that varies per-deploy belongs in environment, never in source |
| 12-Factor App — Factor X (Dev/Prod Parity) | Phase 2: pinned tool versions (`mcp-atlassian==0.21.0`, Python 3.11+) | Same dependency versions across all developer machines eliminates "works on my machine" |
| Idempotent provisioning (Terraform/Ansible pattern) | Phase 1 detection + per-flag skip guards | Each phase checks current state before acting — re-runs converge, never diverge |
| Least-privilege secret handling (OWASP) | Phase 5 step 1: `chmod 600` on `.env`, `chmod 700` on directory; token passed via `--env-file` not argv | Secrets visible in `ps aux` output if passed as argv; file-based injection keeps them out of process list |
| Backup-restore resilience | Phase 1/4: auto-restore config from `~/.config/atlassian/` on reinstall | Plugin reinstalls wipe the cache dir; a backup that survives outside the plugin directory is standard IaC recovery pattern |
...
- **Phases skipped on re-run:** All 5 flags true → 0 interactive phases, straight to Phase 6 step 2 validation. A good idempotent setup converges to "nothing to do" on the second run.
...
- Phase 2 (deps): always checks, installs only if missing — `command -v` before `brew install`
- Phase 3 (config): skipped entirely when `SKIP_CONFIG=true` — existing non-placeholder config is authoritative
- Phase 4 (write): overwrite guard prompts before replacing existing valid config — default is `N` (preserve)
- Phase 5 step 1 (env): skipped when `.env` already has non-empty `JIRA_API_TOKEN` — no re-prompting for credentials
- Phase 5 step 2 (acli auth): skipped when `acli jira auth status` exits 0 — exit code is more stable than string parsing
- Phase 5 step 3 (MCP): skipped when `claude mcp get mcp-atlassian` exits 0 — prevents duplicate registrations

**Configuration drift signals to watch for:**

- `ENV_OK=true` but `ACLI_OK=false` → token in `.env` is stale or expired; re-run Phase 5 steps 1+2
- `MCP_OK=true` but tools return "not found" → MCP registered in a previous session, not yet activated — restart Claude Code
- `VENV_OK=false` after plugin reinstall → expected; Phase 2 always re-syncs; data dir naming changed (`atlassian-pm-atlassian-pm`)
...
| Phase 5 step 2 fails with valid credentials | `acli` reading stale token from OS keychain, not `.env` | Run `acli jira auth logout` then retry Phase 5 step 2 manually |
...
| board_id stays 0 after setup | User skipped Phase 6 step 2 board lookup | Run `/doctor` — it offers board lookup when `board_id=0` is detected |
| mcp-atlassian registered with wrong project filter | `PROJECT_KEY` collected incorrectly in Phase 3 | Remove with `claude mcp remove mcp-atlassian`, fix config, re-run Phase 5 step 3 |
