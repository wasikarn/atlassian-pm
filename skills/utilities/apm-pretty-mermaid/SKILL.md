...
| Use case | Type |
| --- | --- |
| Workflow / process / state machine | `flowchart` or `stateDiagram-v2` |
| API call / service interaction | `sequenceDiagram` |
| Data model / DB schema | `erDiagram` |
| Class / object relationship | `classDiagram` |
| Sprint dependency graph | `flowchart LR` with subgraphs |
