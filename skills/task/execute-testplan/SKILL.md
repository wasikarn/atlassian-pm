...
3. Post execution summary comment on issue_key:
   ...
   **Bugs created:** {{PROJECT_KEY}}-XXXX, BEP-YYYY
   **Sheet:** <sheet_url>
...
Each test execution should record: test case ID, execution date/time, environment, tester, pass/fail, actual results, and incident references. The Google Sheet columns I–L map directly to this standard.

### Test Type Risk Profile

- **Positive tests fail** → Critical (core functionality broken) — P1/High bug
- **Negative tests fail** → Major (system accepts invalid input) — P2/Medium bug
- **Edge tests fail** → Minor (boundary condition) — P3/Low bug

### Headed vs Headless Decision

OAuth flows (LINE, Google, Facebook) cannot be reliably automated headless due to popup blocking and CORS restrictions. Always use headed + manual pause for third-party OAuth steps. Mark as `blocked` if no test account is available.

### Evidence Quality

Screenshots alone are insufficient. Always capture:

1. Screenshot at failure point
2. Console errors (JS exceptions)
3. Network requests (failed API calls)
The combination gives developers enough context to reproduce without manual testing.

### Bug Dedup Threshold

Create new ticket only when confidence < HIGH. Duplicate noise wastes engineering time more than missed bugs. When in doubt, add a comment to the existing ticket instead.

### Sheet Write-back Reliability

Google Sheets cell editing via Playwright is fragile with autosave race conditions. Always verify the "Saving..." spinner disappears before moving to the next cell. For bulk updates, prefer Google Sheets API (`gspread`) if service account credentials are available in env vars (`GOOGLE_SERVICE_ACCOUNT_JSON`).
