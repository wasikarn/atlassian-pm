...
| apm-pretty-mermaid | `/atlassian-pm:apm-pretty-mermaid` | — | node 18+ | Render Mermaid → ASCII for Jira ADF code blocks. Confluence uses native Forge Mermaid macro (no skill needed). ⚠️ 3+ branch flowchart ASCII broken — hand-draw or use `sequenceDiagram`. |
