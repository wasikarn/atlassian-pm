...
- If matches `BEP-\d+` pattern → use as existing epic key, skip create-epic
- Otherwise → pass as description to blueprint

Use the Skill tool to invoke `atlassian-pm:blueprint` with `$ARGUMENTS`.
